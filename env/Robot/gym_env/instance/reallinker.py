...
class Reallinker(Robot):
    ...
    def step(self, action) -> None:
        ...
        if self.cfg.control_type == "effort":
            ...
            right_arm_displacement = action[:, 1:] * 0.05
            right_arm_joint_pos = self.sim.get_joint_pos()[:, 1:] 
            u1 = self.sim.reallinker_right_arm_joint_to_pos(right_arm_displacement, right_arm_joint_pos)

            other_displacement = action[:, :1] * 0
            other_joints_pos = self.sim.get_joint_pos()[:, :1]
            u2 = self.sim.reallinker_other_joint_to_pos(other_displacement, other_joints_pos)
    ...
    def get_obs(self) -> torch.Tensor:
        ...
        ee_position = self.sim.get_reallinker_right_ee_position()
        ee_orientation = self.sim.get_reallinker_right_ee_orientation()
        ee_velocity = self.sim.get_reallinker_right_ee_velocity()
        ee_angular_velocity = self.sim.get_reallinker_right_ee_angular_velocity()
        middle_point_to_object_distance = self.sim.get_realinker_right_hand_to_object_distance() 
        middle_point = self.sim.get_reallinker_right_fingers_mid_point() 
