...
class Reallinker_Grasp_single_object(Task):
    def __init__(self, sim, cfg) -> None:
        ...
        self.right_hand_collision_reset = cfg.reward_scales["right_hand_collision_reset"]
    ...
    def reset_ids(self, env_ids: torch.Tensor) -> torch.Tensor:
        ...
        """只为指定环境重置目标"""
        goals_pos = torch.tensor([0.8, 0, 1.025], dtype=torch.float32, device=self.device)
    ...
    def reward_grasp_mid_point(self):
        two_fingers_mid = self.sim.get_reallinker_right_fingers_mid_point()
    ...
    def reward_pos_reach_distance(self):
        ...
        hand_base_pos = self.sim.get_reallinker_right_ee_position()
    ...
    def reward_right_hand_collision_reset(self):
        reset_events = self.sim.check_reset_events(self.robot_type)
        right_hand_reset = reset_events['right_hand_collision'].float()

        return -self.right_hand_collision_reset * right_hand_reset
